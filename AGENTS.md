...
| Application | `application/{port/in,port/out,usecase}` | domain, Spring for use-case wiring | adapter |
...
- Outbound ports: `*Port` interfaces in `application/port/out/`.
...
- DB: H2 in-memory. Flyway migrations live in `db/migration/incremental/` and
  `db/migration/current/`.
